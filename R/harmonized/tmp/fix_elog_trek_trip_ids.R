...
# print(mismatched %>% left_join(dplyr::select(elog_trek, vessel, trip_id, date), by=c("vessel", "trip_id")))
print(mismatched)
cat("\n")

# ── Re-assign trip_id in elog_trek via date join ──────────────────────────────
cat("Re-assigning trip_ids via vessel + date join to elog...\n")

elog_trek_fixed <- elog_trek %>%
  # Join elog trip_id on vessel + date
  dplyr::left_join(elog_trip_lookup, by = c("vessel", "date")) %>%
  dplyr::mutate(
    trip_id_old = trip_id,
    # Use elog trip_id where available; keep original otherwise
    trip_id = dplyr::if_else(
      !is.na(trip_id_elog),
      trip_id_elog,
      trip_id
    )
  ) %>%
  dplyr::select(-trip_id_elog)
