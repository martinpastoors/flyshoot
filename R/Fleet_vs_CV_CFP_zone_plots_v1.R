...
#
# SOURCES:
#   fishing_days_france  — full fleet with zone_detail (French CFP zones)
...
if (!exists("fishing_days_france")) {
  ...
  fishing_days_france <- local({
    e <- new.env()
    load(file.path(flyshootdir, "gfw_s7_effort.RData"), envir = e)
...
# ==============================================================================
# STEP 2 — Prepare full fleet from fishing_days_france
# ==============================================================================

cv_mmsi <- vessels_cv$mmsi

fishing_days_fleet <- fishing_days_france %>%
