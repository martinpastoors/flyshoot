...
# =====================================================================================================
# FLYSHOOT CPUE analysis - Channel
# 
...
The standardized CPUE for the different species, taking into account the significant factors per species, differ substantially from the raw CPUE. For the four main target species (i.e. the top row in the figures), standardized catch rates for European squid and Surmullet have decreased in the recent period, whereas catch rates for Common cuttlefish and mackerel have increased. For several species, we can observe a maximum in catch rates during the period 2014-2016 and lower catch-rates thereafter. This could point to different targeting behaviour in the fishery and therefore, lower availability of those species to the fishery.

For the three main target species (European squid SQR, Surmullet MUR and Common cuttlefish CTC), the explained deviances are 50, 40 and 66% respectively, whereas the explained deviance for mackerel is only 12%. This indicates that the GLM model for mackerel is not providing a reasonable explanation of the variance in the data, while for the other three it is reasonable to well described. For quota regulated species like whiting, mackerel and horse mackerel, the explained deviance is quite low, probably because the catch rates are constrained by the (limited) quota availability. We also observe some overprediction of catch rates at lower values, for example, for red mullet (surmullet MUR) and gurnards. It is not well understood what drivers lead to these patterns.

Overall, the patterns observed in CPUE for the species shown, make it difficult to interpret the CPUE as a direct indicator of abundance, because the targeting behaviour and the quota limitations may affect the catch rates in different ways in different years. However, the information from the logbooks does provide usable information on the spatial distribution of the fishery and the catches by species. For example, we can see that the spatial distribution of Common cuttlefish catches are quite distinct from most other species, thereby indicating that the CPUE for Cuttlefish may a very different from other species. An important consideration for the overall analysis, is that the logbook information only contains data on the landed catch. This is in fact a severe limitation on the possibility to interpret the results as indicators of abundance because an unknown part of the catch may not have been landed. 
...
t <-
  ...
  # group_by(species) %>% 
  # mutate(tst = n_distinct(year)) %>% 
  # distinct(year)
  
  filter(n_distinct(year) == length(yrs)) %>% 
...
# years %>% group_by(species) %>% summarise(n=n()) %>% View()
...
effects %>% 
  pander::pandoc.table(style="simple", justify="left", split.tables=400) 
...
The standardized CPUE for the different species, taking into account the significant factors per species, are shown in the figure below. If this figure is compared to `r fig_nums("cpue1", display="cite")`, it becomes clear that standardized CPUE may differ from raw CPUE. For the four main target species (i.e. the top row in the figure), catch rates for European squid and Surmullet have decreased in the recent period, whereas catch rates for Common cuttlefish and mackerel have increased. 

For several species, we can observe a maximum in catch rates during the period 2014-2016 and lower catch-rates thereafter. This could point to different targeting behaviour in the fishery and therefore, lower availability of those species to the fishery. 
...
The observed vs. predicted catch rates are a diagnostic to assess how well the GLM model is fitting to the data. Ideally, one would expect the values to be close to the 1:1 line (blue dotted line). The explained deviance (blue text) describes the proportion of the deviance that is explained by the model. The three main target species (SQR, MUR, CTC) have explained deviances of 50, 40 and 66% respectively, whereas the explained deviance for mackerel is only 12%. This indicates that the GLM model for mackerel is not providing a reasonable explanation of the variance in the data, while for the other three it is reasonable to well described. 

For quota regulated species like whiting, mackerel and horse mackerel, the explained deviance is quite low, probably because the catch rates are constrained by the (limited) quota availability. 

For several species, we observe some overprediction of catch rates at lower values, for example, for red mullet (surmullet MUR) and gurnards. It is not well understood what drivers lead to these patterns. 
...
The standardized CPUE for the different species, taking into account the significant factors per species, are shown in `r fig_nums("timetrends", display="cite")`. If that figure is compared to `r fig_nums("cpue1", display="cite")` with the raw catch rates, it is clear that standardized CPUE may differ from raw CPUE. For the four main target species (i.e. the top row in the figures), standardized catch rates for European squid and Surmullet have decreased in the recent period, whereas catch rates for Common cuttlefish and mackerel have increased. For several species, we can observe a maximum in catch rates during the period 2014-2016 and lower catch-rates thereafter. This could point to different targeting behaviour in the fishery and therefore, lower availability of those species to the fishery.

The observed vs. predicted catch rates and the percentage explained deviance are diagnostics to assess how well the GLM model is fitting to the data. For the three main target species (SQR, MUR, CTC), the explained deviances are 50, 40 and 66% respectively, whereas the explained deviance for mackerel is only 12%. This indicates that the GLM model for mackerel is not providing a reasonable explanation of the variance in the data, while for the other three it is reasonable to well described. For quota regulated species like whiting, mackerel and horse mackerel, the explained deviance is quite low, probably because the catch rates are constrained by the (limited) quota availability. We also observe some overprediction of catch rates at lower values, for example, for red mullet (surmullet MUR) and gurnards. It is not well understood what drivers lead to these patterns.

Overall, the patterns observed in CPUE for the species shown, make it difficult to interpret the CPUE as a direct indicator of abundance, because the targeting behaviour and the quota limitations may affect the catch rates in different ways in different years. However, the information from the logbooks does provide usable information on the spatial distribution of the fishery and the catches by species. For example, in `r fig_nums("catch6", display="cite")`, we can see that the spatial distribution of Common cuttlefish catches are quite distinct from most other species, thereby indicating that the CPUE for Cuttlefish may a very different from other species. 
