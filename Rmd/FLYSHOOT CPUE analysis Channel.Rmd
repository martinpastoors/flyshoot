...
dir.create(file.path(onedrive, "reports","tables",folder), showWarnings = FALSE)
...
# p1 <-
  ...
  scale_x_continuous(breaks=seq(min(e$year), max(e$year),1)) +
  scale_y_log10() +
  labs(x="", y="landings/day (kg)", title="Observed landings per week") +
  facet_wrap(~english_species, ncol=4)
...
**Observed revenue rate (revenue / week)**
...
fig_nums(
  name    = "obsrevenue", 
  level = 1, display = FALSE,
  caption = "Observed revenue by week. Red dot indicates medians. Blue line is the linear slope through the medians. The slope refers to the slope of the regression line. ")
...
t %>%   
  ...
  scale_x_continuous(breaks=seq(min(e$year), max(e$year),1)) +
  # scale_y_log10() +
  labs(x="", y="revenue/day (euro)", title="Observed revenue per week") +
...
readr::write_rds(predfinal, file=file.path(onedrive,
                                   "rdata",
