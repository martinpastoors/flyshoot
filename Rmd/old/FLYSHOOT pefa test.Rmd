...
comb <-
  ...
  mutate(dutch_name = ifelse(grepl("dorade", dutch_name), "zeekarper", dutch_name)) %>% 
...
**FLYSHOOT vergelijking Marelec, PEFA exports**
...
Vergelijking Marelec export met oude en nieuwe export faciliteit vanuit PEFA

schip: `r selectvessel`
...
comb %>% 
  ...
  summarise(
    startdate = min(as.Date(catchdate), na.rm=TRUE),
    enddate   = max(as.Date(catchdate), na.rm=TRUE)
  ) %>% 
  pander::pandoc.table(., 
                       style        = "simple",
                       split.tables = 200, 
