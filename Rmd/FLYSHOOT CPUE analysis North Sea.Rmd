...
**Total landings by species and year**
 ...
 Top 15 species in the landings.
...
```{r catch5, echo=FALSE, message=FALSE, warning=FALSE, comment=NA, fig.asp=1.0}
...
t <-
  ...
  left_join(top, by=c("target"= "species")) %>% 
...
t %>% 
  ...
  scale_x_continuous(breaks=seq(min(catch_byweek$year), max(catch_byweek$year),1)) +
  scale_y_continuous(labels=scales::percent) +
  labs(y="", title="Percentage of targeted weeks", x="") +
  facet_wrap(~english_species, ncol=4)
...
contrasts <-
  x0 %>% 
  unnest(data) %>% 
  dplyr::select(one_of("species","efficiency", "var",
                       "vessel", "quarter","month","week","rect","lat","lon", "target")) %>% 
  distinct() %>% 
