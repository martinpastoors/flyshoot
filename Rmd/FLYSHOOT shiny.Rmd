...
shiny::selectInput(inputId = "showby", label = "Show by:",
                   choices = c("by week","by trip"), selected = "by week")
...
#     x = haul[haul$vessel %in% input$setvessel] 
#   })
...
renderPlot(
  ggplot(data=filter(elog, 
           ...
           {if(input$showby == "by week") {
             group_by(., vessel, week, date) 
           } else {
             group_by(., vessel, trip2, date)}} %>% 
    ...
    {if(input$showby == "by week") {
      ...
      geom_bar(aes(fill=trip2), stat="identity") 
    }}+
    labs(x="") +
...
my_vars <- shiny::reactive({
    x =  c("vessel", ifelse(input$showby == "by week", c("week"), c("trip2")))
    })

# renderText(my_vars())

my_showby <- shiny::reactive({
    x =  ifelse(input$showby == "by week", "week", "trip2") 
    })
