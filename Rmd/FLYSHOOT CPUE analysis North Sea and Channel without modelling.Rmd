...
if(!any(is.na(divisions))) {
  ...
  folder <- paste("CPUE northsea-channel", 
                  paste0(year(now()), 
                         stringr::str_pad(month(now()), width=2, pad="0"), 
                         stringr::str_pad(  day(now()), width=2, pad="0") ))
}
...
# e %>% filter(grepl("pilchard", englishname)) %>% distinct(englishname)
...
revenue_byweek <-
  ...
  mutate(revenue = weight * avgprice) %>% 
  drop_na(revenue) %>% 
  group_by(vessel, year, quarter, week, rect, species) %>% 
  summarise(revenue = sum(revenue, na.rm=TRUE)) %>% 
  left_join(icesrect, by="rect") %>% 
  ungroup()
...
**Note: 2023 has been processed up to Date: `r format(max(e$date), '%d/%m/%Y')`**
...
The objective of this work is to analyse the electronic logbook data of the Jaczon Flyshoot vessels for trends in spatial and temporal dynamics for the different target species. The electronic logbook data contains total landings by day, by species and by ICES rectangle. For the current analysis, data for divisions `r paste(sort(unique(catch_byweek$division)), collapse=(", "))` have been used. 
...
Electronic logbook data was available for the vessels `r paste(unique(e$vessel), collapse=", ")` and years  `r paste(c(min(yrs), max(yrs)), collapse="-")`. For the years `r yrs[1]`-2016, data was only available for two vessels (SCH65 and SL9), from 2017 onwards, data became available for the SCH135 and the Z99/SCH99 was added from 2022 onwards.  
...
**Map of total effort by ICES rectangle and year**
...
```{r catch1, echo=FALSE, message=FALSE, warning=FALSE, comment=NA, fig.asp=1.1}
...
**Average price by species and year**

The average price per species and year was derived from the annual sales information of the SL9 and SCH65.  
...
pp <-
  ...
  geom_point(aes(x=year, y=price), colour="darkgrey") +  
  geom_line(aes(x=year, y=price), colour="darkgrey") +  
  geom_text_repel(data=t %>% filter(year==max(yrs)),
                  aes(x=year, y=price, label=format(price,digits=1, nsmall=2)),
                  hjust=0, nudge_x = 0.5) +
  scale_x_continuous(breaks=seq(min(e$year), max(e$year),1), expand=expand_scale(add = c(0, 2))) +
  labs(x="",y="", title="Average price (euro)") +
...
**Total revenue by vessel and year**

Based on landings and average price by year. 

```{r revenue1, echo=FALSE, message=FALSE, warning=FALSE, comment=NA, fig.asp=1.1}

fig_nums(
  name    = "revenue1", 
  level = 1, display = FALSE,
  caption = "Total revenue (euro) per vessel and year (top) and by vessel, year and quarter (bottom)")

my_colors <- RColorBrewer::brewer.pal(12, "Paired")[c(1:4)]
...
p1 <-
  ...
  scale_fill_manual(values = my_colors) +
  ylab(NULL) +
  scale_x_continuous(breaks=seq(min(revenue$year), max(revenue$year),1))
...
_`r fig_nums("revenue1")`_

\newpage

**Map of total revenue by ICES rectangle**
...
**Comparison of Landings and Discards (from total catch estimation)**

A total catch estimator has been implemented on the SCH99. Below are the comparisons of landings and discards per week, whereby discards was derived from the total catch minus the landings. 

```{r landingsdiscards, echo=FALSE, fig.asp=1.0, message=FALSE, warning=FALSE, comment=NA}

fig_nums(
  name    = "totalcatch", 
  level = 1, display = FALSE,
  caption = "Total catch vs landings per week (SCH99)")
...
t <-
  h %>% 
  filter(vessel=="SCH99_Z99") %>% 
  drop_na(catchweight) %>% 
  dplyr::select(-landingweight) %>% 
  left_join(t1, by=c("vessel","trip","haul")) %>% 
  drop_na(landingweight) %>% 
  group_by(vessel, year, week) %>% 
  summarise(
    landingweight = sum(landingweight),
    catchweight   = sum(catchweight)
  ) %>% 
  mutate(
    discardweight = catchweight -landingweight,
    perc_discards = discardweight/catchweight
  )

p1 <-
  t %>% 
  dplyr::select(-perc_discards, -catchweight) %>% 
  ...
  scale_x_continuous(breaks=seq(min(t$week), max(t$week),1)) +
  labs(x="",y="tonnes", title="Landings and estimated discards by week (SCH99)") 

print(p1)

png(filename=file.path(figuresdir, "landings and discards.png"),
     width=12.5, height=5.5, units="in", res=300, bg="transparent")
  print( p1   + 
         theme(plot.background = element_blank()) +
         theme(panel.background = element_blank())) 
...
p2 <-
  ...
  theme(plot.title = element_text(hjust = 0.0)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_bar(aes(x=week, y=perc_discards), stat="identity") +

  scale_x_continuous(breaks=seq(min(t$week), max(t$week),1)) +
  scale_y_continuous(labels=scales::percent) +
  labs(x="",y="percentage discards", title="Calculated percentage discards by week (SCH99)") 

print(p1)

png(filename=file.path(figuresdir, "percentage discards.png"),
     width=12.5, height=5.5, units="in", res=300, bg="transparent")
  print( p2   + 
         theme(plot.background = element_blank()) +
         theme(panel.background = element_blank())) 
...
_`r fig_nums("totalcatch")`_

\newpage

```{r eloghaulcomparison, eval=FALSE, fig.asp=1.4, message=FALSE, warning=FALSE, comment=NA, include=FALSE}
...
t1 <-
  catch_byweek %>% 
  mutate(englishspecies = paste(englishname, species)) %>% 
  filter(englishspecies %in% top$englishspecies) %>%
  filter(year == 2023) %>% 
  group_by(englishspecies, rect, lat, lon, year) %>% 
  summarise(
...
# levels / breaks
lvl <- sort(unique(t1$cpue_interval))
...
An overview of the available data has been presented in the Material and Methods section. The data was available for the vessels SCH65, SL9, SCH135, Z99 and years `r yrs[1]`-`r yrs[length(yrs)]`. For the years 2012-2016, data was only available for two vessels (SCH65 and SL9). From 2017 onwards, data became available for the SCH135 and the Z99 was added in 2022. The data coverage is not complete. The data for 2015-2017 is also incomplete for some vessels. 
