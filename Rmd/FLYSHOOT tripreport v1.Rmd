...
setvessel<-c("Z99", "SCH99")   ;settrip<- c("2026282") ; comparisons <- FALSE
...
if(nrow(h) > 0) {
  lon_range <- range(h$lon, na.rm = TRUE)
  lat_range <- range(h$lat, na.rm = TRUE)
} else {
...
if (!is.null(haul_data)) {
  ...
  p <- 
    ...
    geom_point(aes(x=lon, y=lat, size = landingweight, colour = as.character(trip)),
                alpha = 0.5) +
...
if (sum(!is.na(et$weight)) > 0) {
  ...
  plot_data <-
    ...
    mutate(naam = stringr::str_trim(naam)) %>%
    mutate(species = ifelse(grepl("DICHT",species) & tolower(naam)=="wijting", "WHG", species))
} else {
  cat("No lat - lon information available")
  plot_data <- NULL
}
...
if (!is.null(plot_data)) {
  ...
  myspecies <- plot_data %>% 
    group_by(species) %>% 
    summarise(weight = sum(gewicht, na.rm = TRUE), .groups = "drop") %>% 
    arrange(desc(weight)) %>% 
...
if (nrow(m) > 0) {
  ...
  t <-
    m %>% 
    filter(soorten %in% myspecies2$soorten) %>% 
...
```{r aanvoerendiscards, echo=FALSE, fig.align="center", fig.asp=1.15, message=FALSE, warning=FALSE}
...
```{r eval=FALSE, fig.align="center", fig.asp=1.15, message=FALSE, warning=FALSE, include=FALSE}
