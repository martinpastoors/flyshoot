...
setvessel<-c("Z99", "SCH99")   ;settrip<- c( "2023107","2023108","2023109") ; comparisons <- FALSE
...
if (nrow(m) > 0 & sum(!is.na(h$catchweight))>0) {
  ...
  p <-
    ...
    geom_text(data=tt2,
              aes(x = min(t1$haul), label=paste("Overall discards per trip", perc_discards,"    ")),
...
if (nrow(m) > 0) {
  ...
  s <-
    ...
    mutate(species = ifelse(species == "SQC", "SQC", species)) %>%  
...
ee %>% 
  ...
  scale_linewidth_manual("year", values = c("2020"=.5, "2021"=.5, "2022"=.5, "2023"=1.5), guide = "none") +
  scale_colour_manual("year", values = c("2020"="gray80", "2021"="gray50", "2022"="gray20", "2023"="red")) +
