...
#   dplyr::summarize(weight     = sum(weight, na.rm=TRUE)) %>% 
#   arrange(desc(weight)) %>% 
#   slice_head(n=nspecies_actual)

# e %>% 
#   filter(date == dmy("14/4/2025"), vessel=="SCH135") %>% 
#   View()

# e %>% 
#   filter(is.na(presentation), year==2025) %>% 
#   View()

# e %>%
#   filter(year>= 2023) %>%
#   filter(faozone %in% c("27.7.d", "27.7.e", "27.4.b","27.4.c")) %>% 
#   mutate(area = ifelse(faozone %in% c("27.7.d", "27.7.e"), "channel","northsea")) %>% 
#   group_by(species, scientificname, dutchname, area) %>% 
#   summarise(weight = sum(weight, na.rm=TRUE)) %>% 
#   writexl::write_xlsx(., path="export.xlsx")
...
t <-
  ...
  left_join(
    e %>% 
      ...
      group_by(year, month) %>% 
      summarise(
        # ndiscardedcatch  = sum(!is.na(gewicht)),
...
# # Pre-calculate everything outside the loop
...
for (myspecies in show_species) {
  ...
  # ggsave(
  #   filename = file.path(onedrive, "report", "FLYSHOOT trips overview", 
...
```{r generating_maps2, eval=FALSE, fig.align="center", fig.asp=1.0, message=FALSE, warning=FALSE, include=FALSE}
...
for (myspecies in top$species) {
  ...
  p <-
    ggplot(data=filter(t, species == myspecies)) +
  ...
  png(filename=file.path(onedrive, "report", "FLYSHOOT trips overview", 
                         paste0(myspecies," landings by haul.png")),
       width=6, height=9.8, units="in", res=300)
...
```{r eval=FALSE, fig.align="left", message=FALSE, warning=FALSE, cache=FALSE, include=FALSE}
knitr::include_graphics(file.path(onedrive, "report", "FLYSHOOT trips overview", 
                         paste0("MAC"," landings by haul.png")))
```

```{r, echo=FALSE, fig.align="left", message=FALSE, warning=FALSE, cache=FALSE, include=FALSE}
knitr::include_graphics(file.path(onedrive, "report", "FLYSHOOT trips overview", 
...
# plot catchrate by species and vessel
ee %>% 
