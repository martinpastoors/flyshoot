...
comb <-
  ...
  bind_rows(cpue_channel %>% mutate(source="CPUE", area="channel")) %>% 
  bind_rows(cpue_northsea %>% mutate(source="CPUE", area="northsea")) 
...
t <-
  ...
  filter(species %in% slice_head(top_channel, n=8)$species) %>% 
